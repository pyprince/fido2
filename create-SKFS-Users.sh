...
SAKA_DID=$1
SERVICE_LDAP_BIND_PASS=$2
SERVICE_LDAP_SVCUSER_PASS=$3
SKFS_LDIF_PATH=$4
...
usage() {
        echo "Usage: "
        echo "${0##*/} <did> <bind-pass> <skfs-user-pass> <skfs-ldif-path>"
        echo "Options:"
...
if ! $MYSQL_HOME/bin/mysql -u skfsdbuser -p${MYSQL_SKLES_PASSWORD} skfs -B --skip-column-names -e "select * from domains where did=$SAKA_DID;" | grep "$SAKA_DID" &> /dev/null; then
        >&2 echo -e "\E[31mYou must make a SAKA domain before you can make the SKFS domain.\E[0m"
        exit 1
fi
