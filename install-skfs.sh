...
if [[ ! -z $YUM_CMD ]]; then
	yum -y install unzip libaio ncurses-compat-libs rng-tools curl >/dev/null 2>&1
...
if [ $INSTALL_OPENLDAP = 'Y' ]; then
	...
	if [[ $(cat /etc/system-release | grep -E "CentOS|Red Hat") ]]; then
